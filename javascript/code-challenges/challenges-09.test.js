...
const maxInArray = (arr) => {
  const x = (acc, cur) => {return cur > acc ? cur : acc;};
  return arr.reduce(x);
};
...
const checkValues = (obj, value) => {
  let x = false;
  Object.values(obj).forEach(y => y === value ? x = true : '' );
  return x;
};
...
------------------------------------------------------------------------------------------------ */
// I think i need to take in an object that has key value pairs and combine them into a string that is added to a new array,

const updateNumbers = (obj) => {
  const newArr = [];
  Object.keys(obj).forEach(x => newArr.push(`${x}: ${obj[x]}`));
  return newArr;
};
...
// I have an array filled with objects that contains key value pairs,
// I need to return an array of all the house values,
