...
const count = (target, input) => {
  return input.reduce((acc, cur) => {
    let tally = cur.reduce((inAcc, inCur) => {
      if (inCur === target) {
        return inAcc + 1;
      }
      return inAcc;
...
const divisibleByFiveTwoToThePower = (input) => {
  let result = input.map((x) => {
    return x.filter((y) => typeof y === 'number' && y % 5 === 0)
      .map(z => Math.pow(2, z));
  });
  return result;
};
...
let findMaleAndFemale = (data) => {
  return data.filter(x => x.gender === 'male' || x.gender === 'female')
...
let findShortest = (data) => {
  return data.reduce((x, y) => Number(x.height) < Number(y.height) ? x : y).name;
};
